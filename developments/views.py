...
class DevelopmentUnitModelViewSet(viewsets.ModelViewSet):
    ...
    def floor_plan_detail(self, request, pk=None, plan_id=None):
        unit = self.get_object()
        try:
            plan = unit.floor_plans.get(pk=plan_id)
        except DevelopmentFloorPlan.DoesNotExist:
            return Response(
        ...
        if label is not None:
            plan.label = str(label).strip() or plan.label
        ...
        external_url = request.data.get("external_url")
        if external_url is not None and not upload:
            plan.external_url = str(external_url).strip()
            if plan.external_url and plan.image:
