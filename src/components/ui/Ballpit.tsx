...
class x {
  ...
  dispose() {
    ...
    // No forceContextLoss: en React Strict Mode remonta el mismo canvas y falla.
...
function createBallpit(e, t = {}) {
  const { maxPixelRatio = 1.75, ...config } = t;
  const i = new x({
    canvas: e,
    size: "parent",
    maxPixelRatio,
  ...
  if (!i.renderer) {
    throw new Error("No se pudo crear el renderer WebGL.");
  }
  let s;
...
export default function Ballpit({
  ...
  useEffect(() => {
    ...
    const mount = () => {
      ...
      const ready =
        parent && parent.clientWidth >= 2 && parent.clientHeight >= 2;
