...
function toPx(v: number | string | undefined | null): number {
  if (v == null) return 0;
...
export function PrismaticBurst({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        // @ts-expect-error ogl Mesh dispose helper
        meshRef.current?.remove?.();
      } catch {
        /* ignore dispose errors */
      }
      try {
        // @ts-expect-error ogl Triangle dispose helper
        triRef.current?.remove?.();
      } catch {
        /* ignore dispose errors */
      }
      try {
        // @ts-expect-error ogl Program dispose helper
      ...
      try {
        ...
        const tex = gradTexRef.current as Texture & { texture?: WebGLTexture };
        if (glCtx && tex?.texture) {
          glCtx.deleteTexture(tex.texture);
        }
      } catch {
